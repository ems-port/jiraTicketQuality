...
-- Normalized improvement group items extracted from each grouping payload.
create table if not exists public.improvement_tip_group_items (
    id uuid primary key default gen_random_uuid(),
    grouping_id uuid not null references public.improvement_tip_groupings(id) on delete cascade,
    ...
    created_at timestamptz not null default timezone('utc', now()),
    constraint improvement_tip_group_items_unique unique (grouping_id, group_id)
);

create index if not exists idx_improvement_tip_group_items_grouping
    on public.improvement_tip_group_items (grouping_id, rank_index);
create index if not exists idx_improvement_tip_group_items_topic
    on public.improvement_tip_group_items (topic_key);
create index if not exists idx_improvement_tip_group_items_created
    on public.improvement_tip_group_items (created_at desc);

-- Manager feedback per improvement point (per grouping/group/user).
create table if not exists public.improvement_tip_group_feedback (
    id bigint generated by default as identity primary key,
    ...
    created_at timestamptz not null default timezone('utc', now()),
    updated_at timestamptz not null default timezone('utc', now()),
    constraint improvement_tip_group_feedback_unique unique (grouping_id, group_id, user_id),
    constraint improvement_tip_group_feedback_group_fk foreign key (grouping_id, group_id)
        references public.improvement_tip_group_items(grouping_id, group_id) on delete cascade
);

create index if not exists idx_improvement_tip_group_feedback_group
