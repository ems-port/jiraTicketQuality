  ...
  return (
    <section className="flex flex-col gap-4 rounded-3xl border border-slate-800 bg-slate-900/70 p-6 shadow-inner">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        {hasData ? (
          topGroups.map((group) => <GroupCard key={group.groupId} group={group} />)
        ) : (
  ...
  return (
    <article className="flex flex-col gap-2 rounded-2xl border border-slate-800 bg-slate-900/80 p-4">
      ...
      <p className="text-[11px] uppercase tracking-wide text-slate-500">
        Coverage {metrics.coveragePct?.toFixed?.(1) ?? "0.0"}% · Actionability {metrics.actionabilityScore}/5 ·
        Severity {metrics.severityScore}/5
      </p>
    </article>
  );
}
